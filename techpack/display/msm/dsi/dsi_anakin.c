...
static ssize_t dimming_speed_write(struct file *filp, const char *buff, size_t len, loff_t *off)
{
	...
	if (g_display->panel->panel_is_on) {
		if (strncmp(messages, "1", 1) == 0) {
			rc = dsi_anakin_tx_cmd_set(g_display->panel, DSI_CMD_SET_DIMMING_SPEED_1);
		} else if (strncmp(messages, "20", 20) == 0) {
			rc = dsi_anakin_tx_cmd_set(g_display->panel, DSI_CMD_SET_DIMMING_SPEED_20);
		} else {
...
void dsi_anakin_record_backlight(u32 bl_lvl)
{
	...
	if (g_display->panel->power_mode == 1 && !g_display->panel->allow_panel_fod_hbm) {
		...
		if(g_display->panel->panel_last_backlight > 1) {
			rc = dsi_anakin_tx_cmd_set(g_display->panel, DSI_CMD_SET_AOD_HIGH);
		}
		else if (g_display->panel->panel_last_backlight == 1) {
			rc = dsi_anakin_tx_cmd_set(g_display->panel, DSI_CMD_SET_AOD_LOW);
		}
		// for non 4 / 64 bl && aod on state, prevent display keep off
		else if(g_display->panel->aod_state){
			rc = dsi_anakin_tx_cmd_set(g_display->panel, DSI_CMD_SET_AOD_OTHER);
		}
		if(rc)
