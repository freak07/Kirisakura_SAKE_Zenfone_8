...
int iris_parse_param(struct device_node *np, struct dsi_panel *panel)
{
	...
	pcfg->dsirecover_check_method = CHECK_ONLY_READ;
	pcfg->dsirecover_check_path = PATH_DSI;
...
int iris_enable(struct dsi_panel *panel, struct dsi_panel_cmd_set *on_cmds)
{
	...
	if (iris_get_abyp_mode_nonblocking() == IRIS_PT_MODE) {
		if (lightup_opt & 0x4) /* use one-wire or mipi to switch */
			rc = iris_lightup_exit_abyp(false, true);
		else
			rc = iris_lightup_exit_abyp(true, true);

		if (rc != 0) {
			IRIS_LOGE("Light up exit abyp failed!!!");
			return rc;
		}
		...
		/* restore brightness setting for esd */
		if (pcfg->lp_ctrl.esd_cnt_iris > 0)
			iris_panel_nits_set(0, true, 0);

		if ((lightup_opt & 0x10) == 0) {
			iris_abyp_switch_proc(pcfg->display, IRIS_ABYP_MODE, false);
			if (iris_platform_get() == 0) /* for FPGA */
				iris_abyp_switch_proc(pcfg->display, IRIS_PT_MODE, false);
		}
		...
		struct dsi_mode_info *mode_info)
{
	...
	if (switch_case == SWITCH_PT_TO_PT) {
		vc_id = (switch_cmds->state == DSI_CMD_SET_STATE_LP) ?
			pcfg->vc_ctrl.to_panel_lp_vc_id :
			pcfg->vc_ctrl.to_panel_hs_vc_id;
		rc = iris_dsi_send_cmds(panel, switch_cmds->cmds,
				switch_cmds->count, switch_cmds->state, vc_id);
...
static void _iris_send_cont_splash_pkt(uint32_t type)
{
	...
	if (type == IRIS_CONT_SPLASH_LK) {
		...
		/* kernel simulates UEFI light up, called by debugfs */
		if (iris_get_abyp_mode_nonblocking() != IRIS_PT_MODE)
		...
		int ipopt_cnt)
{
	...
	cmd_comp.cmd_cnt = pcfg->iris_cmds.cmds_index;
	cmd_comp.op_type = IRIS_PQUPDATE_OP;
...
static int iris_dev_probe(struct platform_device *pdev)
{
	int rc = 0;
	struct iris_cfg *pcfg = iris_get_cfg();
...
static int iris_dev_remove(struct platform_device *pdev)
{
...
static struct platform_driver iris_driver = {
	.probe = iris_dev_probe,
	.remove = iris_dev_remove,
	.driver = {
		.name = "pxlw-iris",
