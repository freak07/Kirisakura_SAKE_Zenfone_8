		...
		u32 count, enum dsi_cmd_set_state state, u8 vc_id)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (cmds->msg.rx_buf && cmds->msg.rx_len)
		...
		struct dsi_panel_cmd_set *cmdset)
{
	...
	if (j == cmds_cnt)
		_iris_pt_send_cmds(panel, ptx_cmds, cmds_cnt);
	else
...
static void _iris_i2c_write_panel_cmd(struct dsi_panel *panel, struct dsi_panel_cmd_set *cmdset)
{
	int i = 0;
	int j = 0;
	u32 offset = 0;
	...
	if (j == cmds_cnt)
		_iris_i2c_send_dsi_cmds(ptx_cmds, cmds_cnt);
	else
		IRIS_LOGE("%s(), invalid cmds cnt: %d, j: %d", __func__, cmds_cnt, j);
		...
		struct dsi_panel_cmd_set *cmdset)
{
	...
	if (pcfg->read_path == PATH_I2C)
		_iris_i2c_write_panel_cmd(panel, &local_cmdset);
	else
