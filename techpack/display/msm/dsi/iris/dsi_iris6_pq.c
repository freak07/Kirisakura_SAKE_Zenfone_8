...
void iris_crst_coef_check(const u8 *fw_data, size_t fw_size)
{
	...
	if (iris_crstk_coef_buf == NULL) {
		len = CRSTK_COEF_SIZE * CRSTK_COEF_GROUP;
	...
	memcpy(&iris_crstk_coef_buf[0], (fw_data), (CRSTK_COEF_SIZE * CRSTK_COEF_GROUP));
}
...
void iris_scurve_enable_set(u32 level)
{
	...
	if (level > 0) {
		enable = 1;
		scurvelevel = (0x71 + (level - 1));   //0x71, sCurve
...
int iris_cm_ratio_set(uint8_t chain)
{
	...
	if (pqlt_cur_setting->pq_setting.cmcolortempmode == IRIS_COLOR_TEMP_MANUL)
		...
		if (pqlt_cur_setting->pq_setting.cmcolorgamut == 4)
			value = pcfg->P3_color_temp;
		else if (pqlt_cur_setting->pq_setting.cmcolorgamut == 5)
			value = pcfg->sRGB_color_temp;
		else if (pqlt_cur_setting->pq_setting.cmcolorgamut == 6)
			value = pcfg->hdr_color_temp;
		else if ((pqlt_cur_setting->pq_setting.cmcolorgamut >= 1) &&
				(pqlt_cur_setting->pq_setting.cmcolorgamut <= 3))
			value =  pcfg->vivid_color_temp;
		else
			value = 6500;
	}

	coefBuff_start = pqlt_cur_setting->pq_setting.cmcolorgamut*18*3;
	...
	if (pqlt_cur_setting->pq_setting.cmcolorgamut == 4)
		...
		value_default = pcfg->hdr_color_temp;
			...
			(pqlt_cur_setting->pq_setting.cmcolorgamut <= 3))
		value_default = pcfg->vivid_color_temp;
	else
		value_default = 6500;
...
void iris_clear_init_trigger(void)
{
	...
	if (iris_tm_valid(pqlt_cur_setting->pq_setting.sdr2hdr)) {
		iris_init_update_ipopt_t(IRIS_IP_SYS, 0x20, 0x20, skiplast);
		if (!iris_dynamic_power_get()) {
			if (pqlt_cur_setting->pq_setting.dbc != 0
				|| pqlt_cur_setting->pq_setting.sdr2hdr != 0) {
				iris_init_update_ipopt_t(IRIS_IP_DMA, 0xe2, 0xe2, 1);
				iris_init_update_ipopt_t(IRIS_IP_DMA, 0xe5, 0xe5, 0);
			}
		}
		iris_update_pq_opt();
	}
...
void iris_hdr_ai_input_bl(u32 bl_value)
{
	...
	IRIS_LOGE("hdr ai input back light =%d\n", ai_input_bl_value);
	iris_set_ipopt_payload_data(IRIS_IP_SDR2HDR_2, 0x10, 3, ai_input_bl_value);
	len = iris_update_ip_opt(IRIS_IP_SDR2HDR_2, 0x10, 0x01);

	len = iris_end_hdr();
	IRIS_LOGE("hdr_ai_input_back_light=%d, len=%d\n", bl_value, len);
}
...
void iris_cm_csc_restore(void)
{
	...
	iris_send_ipopt_cmds(IRIS_IP_DPP, 0x40);
